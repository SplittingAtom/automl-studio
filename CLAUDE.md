...
3b. **Export** → `GET /api/models/{id}/export` (`training/export.py`) returns a self-contained scoring kit zip: `model.json` (XGBoost native format, no pickle), `feature_spec.json`, `interval_model.json` (regression), a standalone `predict.py` that REIMPLEMENTS apply_feature_spec (keep the two in sync when preprocessing changes!), and a README. Tests actually run the exported script in a subprocess.
